...
class A2VidPipelineTwoStage:
    ...
    def __call__(  # noqa: PLR0913
        ...
        bar = _tqdm(total=1, desc="Encoding prompt", file=_sys.stderr); bar.refresh()
        ...
        # Encode audio.
        print("  Encoding audio …", file=_sys.stderr)
        bar = _tqdm(total=1, desc="Encoding audio", file=_sys.stderr); bar.refresh()
        ...
        print("  Loading video encoder …", file=_sys.stderr)
        bar = _tqdm(total=1, desc="Loading video encoder", file=_sys.stderr); bar.refresh()
        ...
        bar = _tqdm(total=1, desc="Freeing memory", file=_sys.stderr); bar.refresh()
        del video_encoder
        cleanup_memory()
        bar.update(1); bar.close()
        ...
        print(f"  Upscaling 2x ({width // 2}→{width}, {height // 2}→{height}) …", file=_sys.stderr)
        bar = _tqdm(total=1, desc="Upscaling", file=_sys.stderr); bar.refresh()
        video_encoder = self.stage_1_model_ledger.video_encoder()
        ...
        print(f"  Stage 2: Refinement ({n2} steps, {width}x{height}) …", file=_sys.stderr)
        ...
        bar = _tqdm(total=1, desc="Decoding video", file=_sys.stderr); bar.refresh()
