...
class RetakePipeline:
    ...
    def __call__(  # noqa: PLR0913, PLR0915
        ...
        *,
        ...
        # Ensure samplers.py uses the same (possibly patched) tqdm class
        from ltx_pipelines.utils import samplers as _samplers_mod
        _samplers_mod.tqdm = _tqdm
        ...
        print("  Encoding source video …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Encoding video", file=_sys.stderr); _bar.refresh()
        video_encoder = self.model_ledger.video_encoder()

        # Use av to get metadata
        fps, num_pixel_frames, src_width, src_height = get_videostream_metadata(video_path)

        # For extend: output_frames > source frames — pad latents to match
        target_frames = output_frames if output_frames is not None else num_pixel_frames

        # Encode using source video dimensions (frame_cap = source frames)
        encode_shape = VideoPixelShape(
            batch=1,
        ...
        initial_video_latent = _encode_video_for_retake(
            ...
            video_path=video_path,
            output_shape=encode_shape,
            dtype=dtype,
            device=self.device,
        )

        # Output shape uses target_frames (may be larger for extend)
        output_shape = VideoPixelShape(
            batch=1,
        ...
        # Pad video latent if extending (target > source)
        if target_frames > num_pixel_frames:
            video_scale = SpatioTemporalScaleFactors.default()
            src_latent_frames = (num_pixel_frames - 1) // video_scale.time + 1
            tgt_latent_frames = (target_frames - 1) // video_scale.time + 1
            pad_frames = tgt_latent_frames - src_latent_frames
            if pad_frames > 0:
                pad = torch.zeros(
                    initial_video_latent.shape[0],
                    initial_video_latent.shape[1],
                    pad_frames,
                    initial_video_latent.shape[3],
                    initial_video_latent.shape[4],
                    device=initial_video_latent.device,
                    dtype=initial_video_latent.dtype,
                )
                initial_video_latent = torch.cat([initial_video_latent, pad], dim=2)
        ...
        print("  Freeing video encoder …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Freeing memory", file=_sys.stderr); _bar.refresh()
        del video_encoder
        cleanup_memory()
        _bar.update(1); _bar.close()
        ...
        print("  Encoding source audio …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Encoding audio", file=_sys.stderr); _bar.refresh()
        ...
        print("  Encoding prompt (Gemma 3 12B) …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Encoding prompt", file=_sys.stderr); _bar.refresh()
        ...
        print("  Loading transformer …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Loading transformer", file=_sys.stderr); _bar.refresh()
        ...
        print("  Preparing video latents …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Preparing video", file=_sys.stderr); _bar.refresh()
        ...
        print("  Preparing audio latents …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Preparing audio", file=_sys.stderr); _bar.refresh()
        ...
        print(f"  Denoising ({n_steps} steps, {src_width}x{src_height}) …", file=_sys.stderr)
        ...
        print("  Freeing transformer …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Freeing memory", file=_sys.stderr); _bar.refresh()
        del transformer
        cleanup_memory()
        _bar.update(1); _bar.close()

        print("  Decoding video …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Decoding video", file=_sys.stderr); _bar.refresh()
        decoded_video = vae_decode_video(
            video_state.latent, self.model_ledger.video_decoder(), tiling_config, generator
        )
        _bar.update(1); _bar.close()

        print("  Decoding audio …", file=_sys.stderr)
        _bar = _tqdm(total=1, desc="Decoding audio", file=_sys.stderr); _bar.refresh()
