...
def model_dtype(device: str | torch.device = "cuda") -> torch.dtype:
    """Return bfloat16 for CUDA, float32 for MPS (bfloat16 not supported), bfloat16 for CPU."""
...
def load_flow_model(model_name: str, debug_mode: bool = False, device: str | torch.device = "cuda",
    ...
    if not debug_mode:
        ...
        with torch.device(device):
            return Flux2(FLUX2_MODEL_INFO[model_name.lower()]["params"]).to(dtype)
