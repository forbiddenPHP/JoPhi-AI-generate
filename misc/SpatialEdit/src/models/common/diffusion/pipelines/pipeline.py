...
class Pipeline(DiffusionPipeline):
    ...
    def __init__(
        self,
        vae: AutoencoderKL,
        text_encoder: AutoModelForVision2Seq,
        ...
        if latents is None:
            if reference_images is not None:
                ...
                if enable_denormalization:
                    ref_vae = self.vae.encode(
                        ref_img).latent_dist.sample()
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.autocast(
                    device_type="mps", dtype=target_dtype, enabled=autocast_enabled
                ):
        ...
        if not output_type == "latent":
            ...
            if len(latents.shape) == 5:
                if isinstance(self.vae, AutoencoderKLHunyuanVideo):
                    raise
                    latents = latents.unsqueeze(3)
                    expand_temporal_dim = True
            elif len(latents.shape) == 6:
                pass
            else:
                raise ValueError(
            ...
            with torch.autocast(
                device_type="mps", dtype=vae_dtype, enabled=vae_autocast_enabled
            ):
                if enable_tiling:
                    self.vae.enable_tiling()
