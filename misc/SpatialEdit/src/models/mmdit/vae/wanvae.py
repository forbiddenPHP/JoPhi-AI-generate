...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # apply attention (with MPS head-chunking if needed)
        bt, heads, seq_len, dim_head = q.shape
        max_elements = 2 ** 31
        if q.device.type == "mps" and bt * heads * seq_len * seq_len > max_elements:
            h_chunk = max(1, max_elements // (bt * seq_len * seq_len))
            x = torch.empty_like(q)
            for h_start in range(0, heads, h_chunk):
                h_end = min(h_start + h_chunk, heads)
                x[:, h_start:h_end] = F.scaled_dot_product_attention(
...
def _video_vae(pretrained_path=None, z_dim=None, device='cpu', **kwargs):
    ...
    if pretrained_path.endswith('.safetensors'):
        ...
        pretrained_state_dict = load_file(pretrained_path, device=str(device))
    else:
...
class WanxVAE(nn.Module):
    ...
    def __init__(self,
        ...
        # init model — weights loaded directly on target device, no CPU copy
    ...
    def encode(self, videos, return_posterior=False, **kwargs):
        ...
        """
        with amp.autocast("mps", dtype=torch.float):
            if return_posterior:
